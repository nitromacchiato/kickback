...
export default async function getSchoolIds(){

    // Process Log 
    console.log('Getting all school names')
    ...
    const id_numbers = await result.map(id => {
            ...
            const school_name = id.name.replace(/\s/g, "-")
    ...
    //Process Log 
    console.log('Successfully recieved all school names')
